...
class Button(UIObject):
    def __init__(self, placement: tuple, color: tuple, text: str, alpha: int = 255, font_size: Optional[int] = None,
                 font_color: tuple = (0, 0, 0), font: str = "monospace", click_function: Optional[Callable] = None):
        self.label = None
        super().__init__(placement)
        self.font = font
        ...
        lab_placement = (round(placement[0] + (placement[2] / 8)), round(placement[1] + (placement[3] / 8)),
                         round(placement[2] * (3 / 4)), round(placement[3] * (3 / 4)))
    ...
    def _adjust_label(self):
        ...
        if self.label is not None:
            ...
            self.label.position = (((self.width - self.label.get_width()) / 2) + self.x,
                                   ((self.height - self.label.get_height()) / 2) + self.y)


class Checkbox(UIObject):
    def __init__(self, placement: tuple, checkbox_color: tuple, indicator_color: tuple, text: str,
                 spacing: Optional[int] = None, alpha: int = 255,
                 font: str = "monospace", font_size: Optional[int] = None, font_color: tuple = (0, 0, 0),
        ...
        lab_placement = (self.x + self.height, self.y, self.width - self.height - round(self.height / 4),
    ...
    def draw(self, surface):
        ...
        if self.checked:
            pygame.draw.circle(surface, self.indicator_color,
                               (int(self.x + self.height / 2), int(self.y + self.height / 2)), int(self.height / 4))
    ...
    def _adjust_label(self):
        ...
        if not self.label is None:
            if self.spacing is not None:
                ...
                # Update label position
                self.label.position = (self.x + self.height + self.spacing,
                ...
                # Update label position
                self.label.position = (self.x + self.height + self._calculate_spacing(),
                                       ((self.placement[3] - self.label.get_height()) / 2) + self.placement[1])


class Slider(UIObject):
    def __init__(self, placement: tuple, min_value: Union[int, float], max_value: Union[int, float],
                 jump: Union[int, float], default_value: Union[int, float],
                 slider_color: tuple, bar_color: tuple, text: str, slider_radius: Optional[int] = None,
                 spacing: int = 10, alpha: int = 255,
                 font_size: int = 10, font_color: tuple = (0, 0, 0), font: str = "monospace",
                 click_function: Optional[Callable] = None):
        self.label = None
        super().__init__(placement)
        ...
        if self.slider_radius is not None:
            self.label_pos = (self.x + (self.width / 2 - self.label.get_width() / 2),
                              self.y + self.slider_radius * 2 + self.label.get_height() / 2)
        else:
            self.label_pos = (self.x + (self.width / 2 - self.label.get_width() / 2),
        ...
        if self.placement[2] < (max_value - min_value) / jump:
            raise ValueError(
                "\033[91m Your total amout of jumps is greater than slider width, ex. you can't set slider width to 100, jump to 1 and max value to 200 because there should be at least 1 pixel per value. You can change jump to 2 to correct it \033[0m")
    ...
    def value(self, v: Union[int, float]):
        ...
        # Render new label
        self.label = pygame.font.SysFont(self.font, self.font_size).render(f'{self.text}: {self.value}', 1,
    ...
    def draw(self, surface):
        ...
        center = (int(pixels_per_one_jump * ((self.value - self.min_value) / self.jump) + self.placement[0]),
                  int(self.placement[1] + self.placement[3] / 2))
        if self.slider_radius is not None:
    ...
    def clicked(self, pos: tuple) -> bool:
        ...
        if pos[0] + radius > self.get_absolute_pos()[0] and pos[0] - radius < self.width + self.get_absolute_pos()[0]:
            if pos[1] + radius > self.get_absolute_pos()[1] and pos[1] - radius < self.height + self.get_absolute_pos()[
                1]:
                # Set new value
                ...
                new_value = round(round(distance / pixels_per_one_jump, round_to) * self.jump,
                                  round_to) + self.min_value
...
class Label(UIObject):
    def __init__(self, placement: tuple, text: str, font_size: Optional[int] = None, font_color: tuple = (0, 0, 0),
                 font: str = "monospace", alpha: int = 255):
        super().__init__(placement)
        self.font = font
        self.font_size = font_size
        self.max_font = None
    ...
    def reload_label(self):
        ...
        try:
            ...
            if self.font_size is None:
                ...
                self.label = pygame.font.SysFont(self.font, self.max_font).render(f'{self.text}', 1,
                                                                                  (*self.font_color, self.alpha / 255))
                return
            # Font specified, render normal label
            self.label = pygame.font.SysFont(self.font, self.font_size).render(f'{self.text}', 1,
                                                                               (*self.font_color, self.alpha / 255))
    ...
    def _find_biggest_possible_font(self) -> Optional[int]:
        ...
        testing_label = pygame.font.SysFont(self.font, current_max_font).render(f'{self.text}', 1,
                                                                                (*self.font_color, 1))
